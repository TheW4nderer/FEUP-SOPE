...
void *verify(void *arg) {         
    ...
    while(1){      
        ...
        else if (buf[k] != k)   // detecta valores errados                         
...
/*
1a)Como só um thread corre a secçao critica, o número total da contagem vai ser exatamente o
numero introduzido inicialmente, por isso o resultado é o esperado.

1b)Ao eliminar as chamadas referidas, vao surgir erros, isto deve-se ao facto de varios
threads estarem a correr fill() ao mesmo tempo, incrementando o valor de pos simultaneamente.
Isto faz com que, depois de os threads terminarem, o valor de pos ultrapasse o introduzido, acusando
erros mais tarde, na chamada verify()
